...
class talk_to_svr : public boost::enable_shared_from_this<talk_to_svr>
	...
	void start(ip::tcp::endpoint ep) {
		...
		//绑定时成员函数有一个隐式参数this
		sock_.async_connect(ep, boost::bind(&talk_to_svr::on_connect, shared_from_this(), -1));//创建了async_connect的完成处理句柄；在调用完成处理句柄之前它会保留一个指向talk_to_server实例的智能指针，从而保证当其发生时talk_to_server实例还是保持活动的。
	}
public:
